...
def create_person_table(conn: pg.Connection):
    ...
    try:
        with conn.cursor() as cur:
            # cursor.close() automatically after "with block"
            cur.execute("""
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                        ...
                        FROM person
                        """)
            # I am simply looping over the cursor object instead of fetching the list of rows
            # In case I needed to use the list somewhere else I would store it in a variable
            # as such: people = cur.fetchall()
            print(cur.statusmessage)
            for row in cur:
    ...
    try:
        with conn.cursor() as cur:
            ...
            if balance is not None:
                print('Balance of %s: %d' % (name, balance[0]))
                return balance[0]
            else:
                return None
    except Exception as error:
...
def get_balance_safe(conn:pg.Connection, name:str):
    ...
    try:
        with conn.cursor() as cur:
            balance = cur.execute(query, [name]).fetchone()
            print(cur.statusmessage)
            if balance is not None:
                print('Balance of %s: %d' % (name, balance[0]))
                return balance[0]
...
def transfer_money(conn, acc1:str, acc2:str, amount:int):
    ...
    try:
        with conn.cursor() as cur:
            ...
            print(cur.statusmessage)
            bal1 = cur.fetchone()[0]
            # Here you could check conditions of the transferring person's balance
            # Update balance of person 1
            if bal1 < amount:
                raise ValueError("Insufficient balance for person 1")
            cur.execute("""
                        UPDATE person
                        SET balance = %s
                        WHERE name = %s
